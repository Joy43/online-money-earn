var data, focusBalance, blurBalance, dataFetcher = new XMLHttpRequest(),
    blurTime = Date.now();
...
function exchangePrices() {
    ...
    for (let i = 0; i < convertItems.length; i++) {
        ...
        if (!convertItemValues[i])
            convertItemValues[i] = (inputElement) ? convertItems[i].value : convertItems[i].innerHTML;
        if (inputElement) convertItems[i].removeAttribute('value');
        switch (true) {
            case conversion.currency == 'btc' && convertItems[i].classList.contains('nobtc'):
                dec = 2;
                decStep = 1;
                inputElement ? convertItems[i].value = (convertItemValues[i] * data.rate_usd).toFixed(dec) : convertItems[i].innerHTML = (convertItemValues[i] * data.rate_usd).toFixed(dec) + ' USD';
                break;
            case conversion.currency == 'btc' && convertItems[i].classList.contains('shortbtc'):
                dec = 4;
                inputElement ? convertItems[i].value = (convertItemValues[i] * conversion.rate).toFixed(dec) : convertItems[i].innerHTML = (convertItemValues[i] * conversion.rate).toFixed(dec) + ' ' + conversion.currency.toUpperCase();
                break;
            default:
                inputElement ? convertItems[i].value = (convertItemValues[i] * conversion.rate).toFixed(dec) : convertItems[i].innerHTML = (convertItemValues[i] * conversion.rate).toFixed(dec) + ' ' + conversion.currency.toUpperCase();
                break;
    ...
    for (let i = 0; i < currencyItems.length; i++)
        currencyItems[i].innerHTML = conversion.currency == 'btc' && currencyItems[i].classList.contains('nobtc') ? 'USD' : conversion.currency.toUpperCase();
...
function submitForm(inputs, verify = null) {
    if (!spamLock) {
        ...
        for (let prop in inputs) {
            if (inputs[prop].type !== 'checkbox' || (inputs[prop].type === 'checkbox' && inputs[prop].checked)) {
                if (formData[inputs[prop].name]) {
                    ...
                    formData[inputs[prop].name] = inputs[prop].value;
                }
            }
        ...
        console.log('spamLock');
    }
    console.log('spamLockelse');
    submitData();
}
...
function submitData() {
    ...
    handler.open('POST', '?handle=post', true);
...
function updateBanner() {
    ...
    refImage.src = 'assets/images/' + refBanner.value + '.gif';
